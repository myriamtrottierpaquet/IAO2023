...
data_ajd <- read.csv("cond_IAO_aujourdhui_2023.csv",header = TRUE, sep= ";") ############ VERFIER QUE CEST LE BON FICHIER #######
...
capture_day <- 123 ################################################# VERFIER QUE CEST LE BON JOUR DE CAPTURE #######
catch_no <- 1       ####################### CHANGER AUSSI LE CATCH NUMBER
current_year <- 2023

mod <- lm(weight_size~day, data=filter(cond_cap, year<2022))
...
pred_line <- data.frame(day= filter(cond_cap, year<2022) %>% dplyr::select(day))
pred_line$fit <- predict(mod, data=pred_line)

p1 <- ggplot()+
  geom_smooth(data=filter(cond_cap, year<2022), aes(x=day, y=weight_size), method = lm, se = T)+
  geom_point(data=filter(cond_cap, year<2022), aes(x=day, y=weight_size), col = 'grey')+
...
p2 <- ggplot(data=dat_graph, aes(y=PC1, x=org_weight))+
  ...
  xlab('Point noir = Masse corrigee /n Triangle bleu = Masse reelle')+
  coord_cartesian(xlim=c(2800,2900))
...
chosen_ones <- data.frame(ring_ID = c('2177-57608', '2177-57500', '2177-57616',  #1
                                      '2177-57587', '2127-28083',  #2
                                      '2177-57474', '2177-57703',  #3
                                      '2177-57599', '2197-57595' ), #8
                          pair_no = c(1, 1, 1,  2, 2, 3, 3, 4, 4))                ############################ A MODIFIER SELON LES OIES CHOISIES ######
...
write.table(chosen_to_save, file= paste0("chosen_ones_day_",capture_day, "capture_no_", "1", ".xls"))
